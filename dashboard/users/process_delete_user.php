...
if ($userId === (int)$_SESSION['user_id']) {
    header("Location: users_consult.php?error=self_delete");
    exit();
}
...
if ($action === 'deactivate') {
    ...
    if ($stmt->execute()) {
        $stmt->close();
        $conn->close();
        header("Location: users_consult.php?success=deactivated");
        exit();
    } else {
...
if ($action === 'delete') {
    ...
    if ($stmt->execute()) {
        $stmt->close();
        $conn->close();
        header("Location: users_consult.php?success=deleted");
        exit();
    } else {
